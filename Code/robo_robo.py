...
base_dir='/home/keithmartin/Documents/masters_in_AI/Semester_3/Thesis/data/newer_ data'
static_cost_path=os.path.join(base_dir, "amalgamated_cost_map.csv")
...
# m survivor table 
survivor_path=os.path.join(base_dir, "test_map.csv")
survivor_table = np.genfromtxt(survivor_path, delimiter=",")
...
def simulate_robot():
    ...
    for cost, (end_x, end_y) in cells_with_costs:
        ...
        for i, cell in enumerate(path):
            ...
            # Check if the current cell contains a survivor
            if survivor_table[y][x] == 1:
                current_time = time.time() - start_time
                print(f"Survivor found at cell ({x}, {y}) after {current_time:.2f} seconds")

        # Update the start position for the next iteration
        start_x, start_y = end_x, end_y

    # Print the overall time taken for the robot's traversal
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Overall time taken for the robot's traversal: {elapsed_time:.2f} seconds")
